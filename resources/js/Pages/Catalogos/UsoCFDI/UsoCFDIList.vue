...
const usocfdis = computed(() => usePage().props.usocfdis);
const totalUsoCFDIs = computed(() => usePage().props.totalUsoCFDIs);
...
onMounted(() => {
    initializeDataTable(dataTable, [[1, 'asc']],50);
});
...
watch(
    () => usocfdis.value?.length, // solo reacciona cuando cambia la lista
    () => {
...
const destroy = (usocfdiId) => {
    if (confirm('¿Estás seguro de que deseas eliminar el registro '+usocfdiId+' ?')) {

        const url = route('usocfdi.delete', usocfdiId)
        ...
        router[method](url, {
            preserveScroll: true,
            onSuccess: () => {
                alert('El Uso CFDI '+usocfdiId+' fue eliminado correctamente');
                $("#tblUsoCFDIs-"+usocfdiId).remove();
            },
            onError: (err) => {
                errors.value = err
            },
            ...
            <div class="inline-flex overflow-hidden mb-4 w-full bg-white rounded-lg shadow-md">
                ...
                <div class="px-4 py-2 -mx-3 w-full">
					<span class="mx-3 float-start">
						<span class="font-semibold text-blue-500">({{ totalUsoCFDIs }}) USO CFDI</span>
            ...
            <div class="mt-6 overflow-x-auto">
                    <table id="datasTable" class="w-full whitespace-no-wrap">
                        ...
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50 border-b">
                            ...
                            <th class="px-4 py-3">Clave Uso CFDI</th>
                            <th class="px-4 py-3">Uso CFDI</th>
                            <th class="px-4 py-3"></th>
                        </tr>
                        </thead>
                        <tbody class="bg-white divide-y">
                        <tr v-for="registro in usocfdis" :key="registro.id" class="text-gray-700 delete-button"  :id="'tblUsoCFDIs-' + registro.id" >
                            <td class="px-4 py-3 text-sm">{{ registro.id }}</td>
                            <td class="px-4 py-3 text-sm">{{ registro.clave_usocfdi }}</td>
                            <td class="px-4 py-3 text-sm">{{ registro.usocfdi }}</td>
