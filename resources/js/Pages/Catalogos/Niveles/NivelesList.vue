...
import FormModalNivel from "@/Pages/Catalogos/Niveles/FormModalNivel.vue";
// Finaliza - Bloque de librerias fundamentales

const niveles = computed(() => usePage().props.niveles);

const totalNiveles = computed(() => usePage().props.totalNiveles);
...
const destroy = (itemId) => {
    if (confirm('¿Estás seguro de que deseas eliminar el Item '+itemId+' ?')) {
        ...
        const url = route('nivel.delete', itemId)
        ...
        <div class="p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <div class="mb-6 p-5 bg-indigo-50 rounded-xl border-l-4 border-indigo-600 shadow-xs flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <button @click="openModal(null)" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Agregar nueva nivel">
            ...
            <div class="mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                <!-- Formulario Búsqueda -->
                <form action="/niveles" class="w-full md:w-auto flex gap-2">
                    <input
                        type="text"
                        v-model="textSearch"
                        name="search"
                ...
                <nav class="flex gap-1">
                    <template v-for="(link, index) in niveles.links">
                        <component
                            :is="link.url ? 'a' : 'span'"
                            :href="link.url"
