...
const refreshData = () => {
    router.reload({ only: ['regimenes_fiscales'] });
};
...
const destroy = (regifisId) => {
    if (confirm('¿Estás seguro de que deseas eliminar el registro '+regifisId+' ?')) {

        const url = route('regifis.delete', regifisId)
        ...
        router[method](url, {
            preserveScroll: true,
            onSuccess: () => {
                alert('El registro fiscal '+regifisId+' fue eliminado correctamente');
                $("#tblRegiFis-"+regifisId).remove();
            },
            onError: (err) => {
                errors.value = err
            },
            ...
            <div class="inline-flex overflow-hidden mb-4 w-full bg-white rounded-lg shadow-md">
                ...
                <div class="px-4 py-2 -mx-3 w-full">
					<span class="mx-3 float-start">
						<span class="font-semibold text-blue-500">({{ totalRegFis }}) Regimenes Fiscales</span>
                    ...
                    <FormModalRegimenFiscal
                        :show="showModal"
                        :regimen_fiscal="selectedRegistro"
                    ...
                    <span
                        ...
                        <form action="/regimenes_fiscales_list">
                        <input
                            type="text"
                            name="search"
                            v-model="textSearch"
            ...
            <div class="mt-6 overflow-x-auto">
                    <table id="datasTable" class="w-full whitespace-no-wrap">
                        ...
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50 border-b">
                            ...
                            <th class="px-4 py-3">Clave Fiscal</th>
                            <th class="px-4 py-3">Regimen Fiscal</th>
                            <th class="px-4 py-3"></th>
                        </tr>
                        </thead>
                        <tbody class="bg-white divide-y">
                        <tr v-for="registro in regimenes_fiscales" :key="registro.id" class="text-gray-700 delete-button"  :id="'tblRegiFis-' + registro.id" >
                            <td class="px-4 py-3 text-sm">{{ registro.id }}</td>
                            <td class="px-4 py-3 text-sm">{{ registro.clave_regimen_fiscal }}</td>
                            <td class="px-4 py-3 text-sm">{{ registro.regimen_fiscal }}</td>
