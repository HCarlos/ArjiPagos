...
const fetchGrupos = async () => {
    ...
    try {
        await router.get(
            route(urlAsignados.value, {combo1_id: selectedCombo1.value, combo2_id: selectedCombo2.value}),
            {},
            {
                preserveState: true,
...
onMounted(async () => {
    ...
    // console.log(combo2Keys.length, selectedCombo2.value);
    //
...
const agregarItems = () => {
    ...
    if (!ids.length || !combo1 || !combo2) return
                ...
                <div class="flex-1 overflow-y-auto relative">
                    <div class="absolute inset-0">
                        <template v-if="Object.keys(leftDisponibles).length">
                            <label
                                v-for="(nombre, id) in leftDisponibles"
                                :key="id"
            ...
            <section class="flex-1 flex flex-col bg-white rounded-xl shadow-lg overflow-hidden md:min-w-[360px]">
                <header class="p-4 border-b border-gray-100 space-y-3">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        >
                            <option v-for="(combo1, id) in combo1" :key="id" :value="id">
                                {{ combo1 }}
                            </option>
                        </select>
                        ...
                        <select
                           ...
                           hover:border-gray-300 transition-all"
                        >
                            <option v-for="(combo2, id) in combo2" :key="id" :value="id">
                                {{ combo2 }}
                            </option>
                        </select>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <template v-if="Object.keys(rightAsignados).length">
                        <label
                            v-for="(nombre, id) in rightAsignados"
                            :key="id"
